(function () {
  ...
  function parseAttributes(node, holes, parts, path) {
    ...
    while (i < length) {
      ...
      if (direct || 1 < (sparse = attribute.value.split(UIDC)).length) {
        ...
        /* istanbul ignore else */
        if (!cache.has(name)) {
          var realName = parts.shift().replace(direct ? /^(?:|[\S\s]*?\s)(\S+?)\s*=\s*('|")?$/ :
        ...
        while (i < len) {
          ...
          switch (info.type) {
            ...
            case 'attr':
              ...
              if (sparse === null) updates.push({ fn: fn, sparse: false });else {
                off += sparse.length - 2;
  ...
  let FormElement = function (_Component) {
    ...
    function FormElement({ name, type, label, required }) {
      ...
      const filled = false;
      _this.setState({ name, type, label, required, filled });
      return _this;
    }
    ...
    _createClass(FormElement, [{
      ...
      value: function render() {
        return this.html(_templateObject, this.state.name, this.state.name, this.state.type, this.state.name, this, this.state.filled ? 'filled' : null, this.state.required, this.state.name, this.state.label);
      }
    }]);
  ...
  let LongFormElement = function (_Component2) {
    _inherits(LongFormElement, _Component2);

    _createClass(LongFormElement, [{
      key: 'defaultState',
      get: function () {
        return {
          name: '',
    ...
    function LongFormElement({ name, type, label, required }) {
      _classCallCheck(this, LongFormElement);

      var _this2 = _possibleConstructorReturn(this, (LongFormElement.__proto__ || Object.getPrototypeOf(LongFormElement)).call(this));

      const filled = false;
      _this2.setState({ name, type, label, required, filled });
      return _this2;
    }
    ...
    _createClass(LongFormElement, [{
      ...
      key: 'render',
      value: function render() {
        return this.html(_templateObject2, this.state.name, this.state.name, this, this.state.filled ? 'filled' : null, this.state.required, this.state.name, this.state.label);
      }
    }]);

    return LongFormElement;
  }(Component);

  let AlertElement = function (_Component3) {
    _inherits(AlertElement, _Component3);

    function AlertElement() {
      _classCallCheck(this, AlertElement);

      return _possibleConstructorReturn(this, (AlertElement.__proto__ || Object.getPrototypeOf(AlertElement)).apply(this, arguments));
    }
    ...
    _createClass(AlertElement, [{
      ...
      key: 'render',
      value: function render() {
        return this.html(_templateObject3, this.state.classes, this, this, this.state.message);
      }
    }, {
      key: 'defaultState',
      get: function () {
  ...
  function components () {
    return new Promise(resolve => {
      elements.forEach(item => {
        hyper(document.getElementById(item.el))(_templateObject4, new FormElement(item));
      });

      hyper(document.getElementById('form-message'))(_templateObject4, new LongFormElement({
        name: 'message',
        type: 'textarea',
        label: 'Message',
        required: true
      ...
      hyper(document.getElementById('ajax-message'))(_templateObject4, new AlertElement());
  ...
  document.addEventListener('DOMContentLoaded', () => {
    components().then(() => {
      ...
      Array.from($('#home-clients a, #addresses a, #links a')).forEach(item => {
      ...
      Array.from(accordionMenu).map(checkbox => {
